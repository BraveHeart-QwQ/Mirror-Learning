...
class Invoker
{
    ...
    public bool AreEqual(Type componentType, RemoteCallType remoteCallType, RemoteCallDelegate invokeFunction) =>
        this.componentType == componentType && this.callType == remoteCallType && this.function == invokeFunction;
}
...
public static class RemoteProcedureCalls
{
    ...
    internal static ushort RegisterDelegate(
        ...
        remoteCallDelegates[hash] = new Invoker {
            callType = remoteCallType, componentType = componentType, function = func, cmdRequiresAuthority = cmdRequiresAuthority
        };
        return hash;
    }
    ...
    // to clean up tests
    internal static void RemoveDelegate(ushort hash) => remoteCallDelegates.Remove(hash);
    ...
    static bool GetInvokerForHash(ushort functionHash, RemoteCallType remoteCallType, out Invoker invoker) =>
        remoteCallDelegates.TryGetValue(functionHash, out invoker) && invoker != null && invoker.callType == remoteCallType;
    ...
    internal static bool Invoke(
        ...
        if (GetInvokerForHash(functionHash, remoteCallType, out Invoker invoker) && invoker.componentType.IsInstanceOfType(component)) {
    ...
    ) => GetInvokerForHash(cmdHash, RemoteCallType.Command, out Invoker invoker) && invoker.cmdRequiresAuthority;
    ...
    ) => remoteCallDelegates.TryGetValue(functionHash, out Invoker invoker) ? invoker.function : null;
}
}
