...
public sealed class NetworkIdentity : MonoBehaviour
{
    ...
    NetworkIdentitySerialization lastSerialization = new NetworkIdentitySerialization {
        ownerWriterReliable = new NetworkWriter(),           observersWriterReliable = new NetworkWriter(),
        ownerWriterUnreliableBaseline = new NetworkWriter(), observersWriterUnreliableBaseline = new NetworkWriter(),
        ownerWriterUnreliableDelta = new NetworkWriter(),    observersWriterUnreliableDelta = new NetworkWriter(),
    };
    ...
    internal void SerializeServer_Broadcast(
        ...
        ServerDirtyMasks_Broadcast(
            out ulong ownerMaskReliable, out ulong observerMaskReliable, out ulong ownerMaskUnreliableBaseline,
            out ulong observerMaskUnreliableBaseline, out ulong ownerMaskUnreliableDelta, out ulong observerMaskUnreliableDelta
        );
        ...
        // serialize all components
        // perf: only iterate if either dirty mask has dirty bits.
        if ((ownerMaskReliable | observerMaskReliable | ownerMaskUnreliableBaseline | observerMaskUnreliableBaseline |
             ownerMaskUnreliableDelta | observerMaskUnreliableDelta) != 0) {
            for (int i = 0; i < components.Length; ++i) {
                NetworkBehaviour comp = components[i];
                ...
                if (ownerDirtyReliable || observersDirtyReliable) {
                    ...
                    using (NetworkWriterPooled temp = NetworkWriterPool.Get()) {
                        ...
                        if (ownerDirtyReliable)
                            ownerWriterReliable.WriteBytes(segment.Array, segment.Offset, segment.Count);
                        if (observersDirtyReliable)
                            observersWriterReliable.WriteBytes(segment.Array, segment.Offset, segment.Count);
                    }
                ...
                if (ownerDirtyUnreliableDelta || observersDirtyUnreliableDelta) {
                    using (NetworkWriterPooled temp = NetworkWriterPool.Get()) {
                        ...
                        if (ownerDirtyUnreliableDelta)
                            ownerWriterUnreliableDelta.WriteBytes(segment.Array, segment.Offset, segment.Count);
                        if (observersDirtyUnreliableDelta)
                            observersWriterUnreliableDelta.WriteBytes(segment.Array, segment.Offset, segment.Count);
                ...
                if (unreliableBaseline && (ownerDirtyUnreliableBaseline || observersDirtyUnreliableBaseline)) {
                    using (NetworkWriterPooled temp = NetworkWriterPool.Get()) {
                        ...
                        if (ownerDirtyUnreliableBaseline)
                            ownerWriterUnreliableBaseline.WriteBytes(segment.Array, segment.Offset, segment.Count);
                        if (observersDirtyUnreliableBaseline)
                            observersWriterUnreliableBaseline.WriteBytes(segment.Array, segment.Offset, segment.Count);
                    }
    ...
    internal NetworkIdentitySerialization GetServerSerializationAtTick(int tick, bool unreliableBaselineElapsed) {
        ...
        if (lastSerialization.tick != tick
            ...
            SerializeServer_Broadcast(
                lastSerialization.ownerWriterReliable, lastSerialization.observersWriterReliable,
                lastSerialization.ownerWriterUnreliableBaseline, lastSerialization.observersWriterUnreliableBaseline,
                lastSerialization.ownerWriterUnreliableDelta, lastSerialization.observersWriterUnreliableDelta, unreliableBaselineElapsed
            );
