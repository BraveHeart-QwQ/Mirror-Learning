...
public static partial class NetworkClient
{
    ...
    internal static void OnTransportData(ArraySegment<byte> data, int channelId) {
        if (connection != null) {
            ...
            while (!isLoadingScene && unbatcher.GetNextMessage(out ArraySegment<byte> message, out double remoteTimestamp)) {
    ...
    internal static void RegisterMessageHandlers(bool hostMode) {
        ...
        if (hostMode) {
            ...
            RegisterHandler<NetworkPongMessage>(
                _ => {}, false
            );
            RegisterHandler<SpawnMessage>(OnHostClientSpawn);
            // host mode doesn't need spawning
            RegisterHandler<ObjectSpawnStartedMessage>(
                _ => {}
            );
            // host mode doesn't need spawning
            RegisterHandler<ObjectSpawnFinishedMessage>(
                _ => {}
            );
            // host mode doesn't need state updates
            RegisterHandler<EntityStateMessage>(
                _ => {}
            );
            RegisterHandler<EntityStateMessageUnreliableBaseline>(
                _ => {}
            );
            RegisterHandler<EntityStateMessageUnreliableDelta>(
                _ => {}
            );
        } else {
    ...
    public static void RegisterHandler<T>(Action<T> handler, bool requireAuthentication = true)
        ...
        handlers
        [msgType] = NetworkMessages.WrapHandler((Action<NetworkConnection, T>)HandlerWrapped, requireAuthentication, exceptionsDisconnect);
    }
    ...
    public static void RegisterHandler<T>(Action<T, int> handler, bool requireAuthentication = true)
        ...
        handlers[msgType] =
            NetworkMessages.WrapHandler((Action<NetworkConnection, T, int>)HandlerWrapped, requireAuthentication, exceptionsDisconnect);
    }
    ...
    // TODO why do we have one with SpawnDelegate and one with SpawnHandlerDelegate?
    public static void
    RegisterPrefab(GameObject prefab, uint newAssetId, SpawnHandlerDelegate spawnHandler, UnSpawnDelegate unspawnHandler) {
        if (newAssetId == 0) {
    ...
    static void OnEntityStateMessage(EntityStateMessage message) {
        ...
        if (spawned.TryGetValue(message.netId, out NetworkIdentity identity) && identity != null) {
            using (NetworkReaderPooled reader = NetworkReaderPool.Get(message.payload)) identity.DeserializeClient(reader, false);
        } else
            Debug.LogWarning(
    ...
    static void OnEntityStateMessageUnreliableBaseline(EntityStateMessageUnreliableBaseline message, int channelId) {
        ...
        if (channelId != Channels.Reliable) {
            Debug.LogError(
                $"Client OnEntityStateMessageUnreliableBaseline arrived on channel {channelId} instead of Reliable. This should never happen!"
            );
            return;
        }
    ...
    static void OnEntityStateMessageUnreliableDelta(EntityStateMessageUnreliableDelta message, int channelId) {
        ...
        if (channelId != Channels.Unreliable) {
            Debug.LogError(
                $"Client OnEntityStateMessageUnreliableDelta arrived on channel {channelId} instead of Unreliable. This should never happen!"
            );
            return;
        }
        ...
        if (spawned.TryGetValue(message.netId, out NetworkIdentity identity) && identity != null) {
            ...
            // or even intentionally, if unreliableRedundancy is turned on.
                ...
                if (!unreliableRedundancy)
                    Debug.Log(
                        $"Client caught duplicate Unreliable state message for {identity.name}. This is fine.\nIdentity timestamp={identity.lastUnreliableStateTime:F3} batch remoteTimestamp={connection.remoteTimeStamp:F3}"
                    );
                return;
            }
            ...
            if (message.baselineTick != identity.lastUnreliableBaselineReceived) {
                Debug.Log(
                    $"Client caught Unreliable state message for old baseline for {identity} with baselineTick={identity.lastUnreliableBaselineReceived} messageBaseline={message.baselineTick}. This is fine."
                );
                return;
            }
        ...
        if (FindOrSpawnObject(message, out NetworkIdentity identity)) {
            if (isSpawnFinished) {
                ...
                SpawnMessage messageCopy = new SpawnMessage {
                    ...
                    sceneId = message.sceneId,       assetId = message.assetId, position = message.position,
                    rotation = message.rotation,     scale = message.scale,     payload = new ArraySegment<byte>(payloadCopy)
                };
                spawned[message.netId]  = identity;
    ...
    static void BroadcastToServer(bool unreliableBaselineElapsed) {
        ...
        foreach (NetworkIdentity identity in connection.owned) {
            ...
            if (identity != null) {
                ...
                using (
                    NetworkWriterPooled writerReliable = NetworkWriterPool.Get(), writerUnreliableDelta = NetworkWriterPool.Get(),
                    ...
                    if (writerReliable.Position > 0) {
                        ...
                        EntityStateMessage message =
                            new EntityStateMessage { netId = identity.netId, payload = writerReliable.ToArraySegment() };
                        Send(message);
                    }
