...
public static partial class NetworkServer
{
    ...
    public static float unreliableBaselineInterval =>
        unreliableBaselineRate < int.MaxValue ? 1f / unreliableBaselineRate : 0;  // for 1 Hz, that's 1000ms
    ...
    [Obsolete(
        "NetworkServer.dontListen was replaced with NetworkServer.listen. The new value is the opposite, and avoids double negatives like 'dontListen=false'"
    )]
...
#pragma warning restore CS0618  // Type or member is obsolete
        ...
        if (!conn.isReady) {
            ...
            if (channelId == Channels.Reliable) {
                ...
                if (RemoteProcedureCalls.GetFunctionMethodName(msg.functionHash, out string method)) {
                    Debug.LogWarning(
                        $"Command {method} received from {conn} when client was not ready.\nThis may be ignored if client intentionally set NotReady."
                    );
                    return;
                }

                Debug.LogWarning(
                    $"Command received from {conn} while client is not ready.\nThis may be ignored if client intentionally set NotReady."
                );
            }
            return;
        }
    ...
    // for client's owned ClientToServer components.
    static void
    OnEntityStateMessageUnreliableBaseline(NetworkConnectionToClient connection, EntityStateMessageUnreliableBaseline message, int channelId) {
        // safety check: baseline should always arrive over Reliable channel.
        if (channelId != Channels.Reliable) {
            Debug.LogError(
                $"Server OnEntityStateMessageUnreliableBaseline arrived on channel {channelId} instead of Reliable. This should never happen!"
            );
            return;
        }
            ...
            if (identity.connectionToClient == connection) {
                ...
                using (NetworkReaderPooled reader = NetworkReaderPool.Get(message.payload)) {
                    ...
                    if (!identity.DeserializeServer(reader, true)) {
                        if (exceptionsDisconnect) {
                            Debug.LogError(
                                $"Server failed to deserialize client unreliable state for {identity.name} with netId={identity.netId}, Disconnecting."
                            );
                            connection.Disconnect();
                        } else
                            Debug.LogWarning(
                                $"Server failed to deserialize client unreliable state for {identity.name} with netId={identity.netId}."
                            );
                    }
                }
            }
    ...
    // for client's owned ClientToServer components.
    static void
    OnEntityStateMessageUnreliableDelta(NetworkConnectionToClient connection, EntityStateMessageUnreliableDelta message, int channelId) {
        // safety check: baseline should always arrive over Reliable channel.
        if (channelId != Channels.Unreliable) {
            Debug.LogError(
                $"Server OnEntityStateMessageUnreliableDelta arrived on channel {channelId} instead of Unreliable. This should never happen!"
            );
            return;
        }
            ...
            if (identity.connectionToClient == connection) {
                ...
                // or even intentionally, if unreliableRedundancy is turned on.
                    ...
                    if (!unreliableRedundancy)
                        Debug.Log(
                            $"Server caught duplicate Unreliable state message for {identity.name}. This is fine.\nIdentity timestamp={identity.lastUnreliableStateTime:F3} batch remoteTimestamp={connection.remoteTimeStamp:F3}"
                        );
                    return;
                }
                ...
                if (message.baselineTick != identity.lastUnreliableBaselineReceived) {
                    Debug.Log(
                        $"Server caught Unreliable state message for old baseline for {identity} with baselineTick={identity.lastUnreliableBaselineReceived} messageBaseline={message.baselineTick}. This is fine."
                    );
                    return;
                }
    ...
    public static bool RemoveConnection(int connectionId) => connections.Remove(connectionId);
    ...
    public static void
              SendToReadyObservers<T>(NetworkIdentity identity, T message, bool includeOwner = true, int channelId = Channels.Reliable)
        where T : struct, NetworkMessage {
    ...
    internal static void OnTransportData(int connectionId, ArraySegment<byte> data, int channelId) {
        if (connections.TryGetValue(connectionId, out NetworkConnectionToClient connection)) {
            ...
            while (!isLoadingScene && connection.unbatcher.GetNextMessage(out ArraySegment<byte> message, out double remoteTimestamp)) {
    ...
    public static bool
    ReplacePlayerForConnection(NetworkConnectionToClient conn, GameObject player, uint assetId, bool keepAuthority = false) {
        if (GetNetworkIdentity(player, out NetworkIdentity identity))
            identity.assetId = assetId;
    ...
    public static bool
    ReplacePlayerForConnection(NetworkConnectionToClient conn, GameObject player, uint assetId, ReplacePlayerOptions replacePlayerOptions) {
        if (GetNetworkIdentity(player, out NetworkIdentity identity))
            identity.assetId = assetId;
    ...
    // This does NOT change the ready state of the connection, so it can safely be used while changing scenes.
    public static bool
    ReplacePlayerForConnection(NetworkConnectionToClient conn, GameObject player, ReplacePlayerOptions replacePlayerOptions) {
        if (!player.TryGetComponent(out NetworkIdentity identity)) {
    ...
    public static void
    RemovePlayerForConnection(NetworkConnectionToClient conn, RemovePlayerOptions removeOptions = RemovePlayerOptions.KeepActive) {
        if (conn.identity == null)
            return;
    ...
    internal static void SendSpawnMessage(NetworkIdentity identity, NetworkConnectionToClient conn) {
        ...
        using (NetworkWriterPooled ownerWriter = NetworkWriterPool.Get(), observersWriter = NetworkWriterPool.Get()) {
            ...
            SpawnMessage       message = new SpawnMessage {
                netId = identity.netId, isLocalPlayer = conn.identity == identity, isOwner = isOwner, sceneId = identity.sceneId,
                assetId = identity.assetId,
                // use local values for VR support
                position = identity.transform.localPosition, rotation = identity.transform.localRotation,
                scale = identity.transform.localScale, payload = payload
            };
            conn.Send(message);
        }
    }

    static ArraySegment<byte>
    CreateSpawnMessagePayload(bool isOwner, NetworkIdentity identity, NetworkWriterPooled ownerWriter, NetworkWriterPooled observersWriter) {
    ...
    internal static void SendChangeOwnerMessage(NetworkIdentity identity, NetworkConnectionToClient conn) {
        ...
        conn.Send(new ChangeOwnerMessage {
            netId = identity.netId, isOwner = identity.connectionToClient == conn,
    ...
    ) => identity.transform.parent == null || identity.transform.parent.gameObject.activeInHierarchy;
    ...
    SerializeForConnection_ReliableComponents(NetworkIdentity identity, NetworkConnectionToClient connection, bool unreliableBaselineElapsed) {
    ...
    static void BroadcastToConnection(NetworkConnectionToClient connection, bool unreliableBaselineElapsed) {
        ...
        foreach (NetworkIdentity identity in connection.observing) {
            ...
            if (identity != null) {
                ...
                NetworkWriter serialization = SerializeForConnection_ReliableComponents(
                    identity, connection,
                ...
                if (serialization != null) {
                    EntityStateMessage message = new EntityStateMessage { netId = identity.netId, payload = serialization.ToArraySegment() };
                ...
                if (deltaSerialization != null) {
                    EntityStateMessageUnreliableDelta message = new EntityStateMessageUnreliableDelta {
                        baselineTick = identity.lastUnreliableBaselineSent, netId = identity.netId,
    ...
    internal static void NetworkLateUpdate() {
        if (active) {
            ...
            bool unreliableBaselineElapsed =
                AccurateInterval.Elapsed(NetworkTime.localTime, unreliableBaselineInterval, ref lastUnreliableBaselineTime);
