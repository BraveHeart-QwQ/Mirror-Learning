...
public class NetworkRoomManager : NetworkManager
{
    ...
    public void CheckReadyToBegin() {
        ...
        int numberOfReadyPlayers = NetworkServer.connections.Count(
            conn => conn.Value != null && conn.Value.identity != null && conn.Value.identity.TryGetComponent(out NetworkRoomPlayer nrp) &&
                    nrp.readyToBegin
        );
        ...
        if (newSceneName == RoomScene) {
            foreach (NetworkRoomPlayer roomPlayer in roomSlots) {
                ...
                if (NetworkServer.active) {
                    ...
                    NetworkServer
                        .ReplacePlayerForConnection(identity.connectionToClient, roomPlayer.gameObject, ReplacePlayerOptions.KeepAuthority);
                }
            }
